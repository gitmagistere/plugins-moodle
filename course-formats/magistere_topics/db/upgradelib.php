...
/**
 * Upgrade scripts for course format "Magistere Topics"
 *
...
/**
 * This method finds all courses in 'magistere_topics' format that have actual number of sections
 * different than their 'numsections' course format option.
 *
 * For courses where there are more sections than numsections, we call
 * {@link format_magistere_topics_upgrade_hide_extra_sections()} and
 * either delete or hide "orphaned" sections. For courses where there are fewer sections
 * than numsections, we call {@link format_magistere_magistere_topics_upgrade_add_empty_sections()} to add
 * these sections.
 */
function format_magistere_magistere_topics_upgrade_remove_numsections() {
    global $DB;
    ...
    $params = ['format1' => 'magistere_topics', 'format2' => 'magistere_topics', 'numsections' => 'numsections'];
    ...
    foreach ($needfixing as $courseid => $numsections) {
        format_magistere_topics_upgrade_hide_extra_sections($courseid, $numsections);
    }

    foreach ($needsections as $courseid => $numsections) {
        format_magistere_topics_upgrade_add_empty_sections($courseid, $numsections);
    }

    $DB->delete_records('course_format_options', ['format' => 'magistere_topics', 'sectionid' => 0, 'name' => 'numsections']);
}
 ...
 */
function format_magistere_topics_upgrade_hide_extra_sections($courseid, $numsections) {
    global $DB;
 ...
 * @param int $numsections
 */
function format_magistere_topics_upgrade_add_empty_sections($courseid, $numsections) {
    global $DB;
